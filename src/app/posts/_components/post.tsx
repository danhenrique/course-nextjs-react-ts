...
export async function Post({ post, showLink = true }: { post: PostProps, showLink?: boolean }) {
  ...
  return (
    <>
      <div key={post.id} className="max-w-2xl px-8 py-4 bg-white rounded-lg shadow-md dark:bg-gray-800">
        ...
        <div className="flex items-center justify-between mt-4">
          <div>
            {showLink && (
              <Link href={`/posts/${id}`} className="text-blue-600 dark:text-blue-400 hover:underline">Read more</Link>
            )}
          </div>
